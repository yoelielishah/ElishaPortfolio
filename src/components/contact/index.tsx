...
// Defining the FormData interface to type the form data
interface FormData {
  name: string;
  email: string;
  message: string;
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      if (axios.isAxiosError(err)) {
        // If the error is an AxiosError, we can access the response data
        setError(err.response?.data?.error || err.message || 'An Unexpected Error Occured.');
          ...
          <>
            <div className="mb-1 mt-6">
              <input
                ...
                {...register('name')}
                className={`appearance-none border transition duration-500 ease-in-out ${errors.name ? 'border-orange-500' : 'border-zinc-500/70 dark:border-zinc-500/40'
            ...
            <div className="mb-2">
              <input
                ...
                {...register('email')}
                className={`appearance-none border transition duration-500 ease-in-out ${errors.email ? 'border-orange-500' : 'border-zinc-500/70 dark:border-zinc-500/40'
                  } font-mplus rounded-lg font-light text-sm w-full py-3 px-3 text-gray-700 dark:text-zinc-50 dark:bg-zinc-800/10 leading-tight focus:outline-none focus:shadow-outline placeholder:font-normal`}
              />
              {errors.email && (
            ...
            <div className="mb-4">
              <textarea
                ...
                rows={4}
                {...register('message')}
                className={`appearance-none border transition duration-500 ease-in-out ${errors.message ? 'border-orange-500' : 'border-zinc-500/70 dark:border-zinc-500/40'
            ...
            <div className="flex items-center justify-center">
              <button
                className={`font-mplus rounded-lg font-semibold text-sm w-full py-3 px-3 text-zinc-50 leading-tight bg-teal-900 p-3 hover:shadow-zinc-900/20 dark:hover:shadow-teal-200/5 hover:shadow-lg transition ease-in duration-100${loading && 'opacity-50 cursor-not-allowed'
                  }`}
                type="submit"
                disabled={loading}
              >
